...
from .forms import MovieForm, UserRegisterForm
...
def insert_data_submission(request):
    ...
    try:
        ...
        new_movie = Movie(title=title, year=year, genres=genres, description=description, director=director_name,
                          actor=actor_name)
        new_movie.save()
        return render(request, "insert_data.html", {})
    except ValueError:
        messages.success(request, f'Please type something valid {username}!')
        return render(request, "insert_data.html")
